...
A simple bespoke (i.e. I made it up with no real plan) 8-bit system running a Z80 CPU with a 40x30 character display, each character can be set to one of 256 colours.  No sound capabilities.  Hardware signals and MiSTer compatible inputs are mapped into memory regions.
...
### Components
 ...
 - 3 Memory-mapped IO regions (all read-only).  _Yes I know about MREQ but SFRs turned out to be annoying in sdcc so :)_
...
0x8000|0x87FF|0x0800|Char index RAM
0x8800|0x8FFF|0x0800|Char foreground colour RAM
0x9000|0x97FF|0x0800|Char background colour RAM
